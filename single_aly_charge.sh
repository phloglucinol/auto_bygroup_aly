...
system=`echo ${ROOT##*/}`
base_path=`echo ${ROOT%/*}`
aly_ele_type='all_mol_ene' # could be intra_mol_ene or all_mol_ene or mol_env_ene
        ...
        if [[ -e segment_md_finished || -e MD_Finish ]]; then
            ...
            if [ -e copy_finished ]; then
                ...
                if [ $sorted_single_dir == 'restraints' ]; then
                    ...
                    python $auto_aly_path/charge_lambda_csv_processor.py
                    cp process_data/${aly_ele_type}_data/*.csv ./sample_csv_data 
                fi
                #touch copy_finished
            fi
        cd sample_csv_data
